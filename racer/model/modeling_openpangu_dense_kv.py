...
class PanguEmbeddedDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: PanguEmbeddedConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = PanguEmbeddedAttention(config=config)
...
class PanguEmbeddedModel(PanguEmbeddedPreTrainedModel):
    def __init__(self, config: PanguEmbeddedConfig):
        ...
        self.layers = nn.ModuleList(
            [PanguEmbeddedDecoderLayer(config) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        for idx, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            # [MODIFIED] get low- mid- and high-level hidden states
            if output_hidden_states and (idx==len(self.layers)-3 or idx==len(self.layers)//2 or idx==2):
    ...
    # [MODIFIED] Copied from modeling_qwen3_kv.py
