...
class LlamaRotaryEmbedding(torch.nn.Module):
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        # Different from paper, but it uses a different permutation in order to obtain
...
class LlamaDynamicNTKScalingRotaryEmbedding(LlamaRotaryEmbedding):
    """LlamaRotaryEmbedding extended with Dynamic NTK scaling. Credits to the Reddit
    users /u/bloc97 and /u/emozilla"""
...
class Llama3Eagle3Drafter(BaseEagle3Drafter):
    def __init__(
        self,
        config,
        load_emb=False,
        ...
        if "midlayer.self_attn.q_proj.weight" in state_dict:
            ...
            for k, v in state_dict.items():
                if k.startswith("midlayer."):
                    new_k = k.replace("midlayer.", "layers.0.")
